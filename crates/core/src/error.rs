...
pub enum CoreError {
    ...
    /// `rng_state` BLOB encode failure.
    Encode(bincode::error::EncodeError),
    /// `rng_state` BLOB decode failure.
...
impl fmt::Display for CoreError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Encode(e) => write!(f, "rng_state encode error: {e}"),
            Self::Decode(e) => write!(f, "rng_state decode error: {e}"),
